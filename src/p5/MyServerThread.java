...
public class MyServerThread extends Thread
{
    ...
    */
	...
	public MyServerThread(Socket mySocket, ArrayList <Socket> connectedSockets)
	{
	   /*
	...
	public void run ()
	{
		...
		while (true) {		
			...
			try {
				...
				System.out.println(sm.sender + " @ socket: " + connectedSockets.get(connectedSockets.size()-1) + ":");     //Sender of the package is shown
				...
				if(sm.receiver==null) {             //If new client joins, then the sent reciever is set to null
					...
					for(int i = 0; i < connectedSockets.size(); i++) {
						try {
							clientOutput = new ObjectOutputStream(connectedSockets.get(i-1).getOutputStream());
							clientOutput.writeObject(sm);           //The object that was passed to server is then pushed to the other client
							clientOutput.flush();
						}catch(Exception e) {
							System.out.println("Error inside for loop: " + e);
						}
					}
				}
			...
			}catch (Exception e) {System.out.println("at continue " + e); continue;}
		}
	}	
}